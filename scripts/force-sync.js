...
async function main() {
  ...
  try {
    ...
    // Body 95 Text
    const body95 = `Dear {{contactName}},\n\nThe payment against the Invoice {{invoiceNumber}} Dated {{billDate}} of amount Rs. {{amount}} is now 90 days overdue. \n\nAs per our company policy, your invoicing will be put on hold effective today, as the outstanding payment has not been cleared within the 90-day credit period.\n\nSo please arrange to remit the outstanding payment immediately to ensure the continuation of supplies and the resumption of invoicing.\n\nThank you for your attention in the matter.\n\nRegards,\nARB Bearings Limited`;

    if (doc && doc.templates) {
      doc.templates.forEach(t => {
        if (t.name === '90 Day Reminder') {
          t.emailBody = body90;
          t.whatsappBody = `INVOICE {{invoiceNumber}} | Dear {{contactName}}, Invoice {{invoiceNumber}} Dated {{billDate}} of Rs. {{amount}} is significantly overdue. Pay within 5 days or future invoicing will be on hold. — {{senderCompany}}`;
          t.smsBody = t.whatsappBody;
          updated = true;
        }
        if (t.name === '95 Day Reminder') {
          t.emailBody = body95;
          t.whatsappBody = `INVOICE {{invoiceNumber}} | Dear {{contactName}}, Invoice {{invoiceNumber}} Dated {{billDate}} of Rs. {{amount}} is 90 days overdue. Invoicing will be on hold effective today. — ARB Bearings Limited`;
          t.smsBody = t.whatsappBody;
          updated = true;
        }
      });
